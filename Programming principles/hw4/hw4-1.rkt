...
(define (list-remove-zero mlist) ;
  (define (list-remove-aux orig-lst lst) ;list-remove-zero(recursion): ends when orig-list becomes null(size of 0), else reduces the size of orig-list by 1 and change the original-list to a zero-deleted list 
...
(define (tree-traversal tree lst) ;tree-traversal(recursion): ends when it meets leaf, else it goes down the tree from root. It must end because each tree have leafs.  
...
(define (make-tree lst) ;make-tree(recursion): ends when the length of list(lst) becomes 1(when it becomes tree), else it reduces size of the list by 1(list length n -> n-1; n>0). 
